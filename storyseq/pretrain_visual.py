"""Level 1: pretrain the visual autoencoder with a reconstruction loss on every training frame.

It gives the sequence predictor an image encoder whose
...
Reports validation L1 (test-split frames) next to the constant-median floor, and writes
  pretrained/visual_ae.pt (or visual_ae_w<width>.pt)   weights
  pretrained/reconstructions.png                       originals vs reconstructions
Run from the repository root: python -m storyseq.pretrain_visual [--epochs 12] [--width 2 --extra groundcap]
"""
...
from storyseq.components import VisualAutoencoder
from storyseq.data import all_frames, load_split
...
def main() -> None:
    ...
    ap.add_argument("--out", default="", help="weights file (default pretrained/visual_ae.pt, or visual_ae_w<width>.pt)")
    args = ap.parse_args()
    torch.manual_seed(0)

"""Level 2: pretrain the text decoder as an unconditional language model on every cached description.
...
zero condition on all of them, reports test perplexity, and saves pretrained/text_lm.pt, which
the trainer loads into the decoder (embedding, LSTM, output layer) before conditional training.

Run from the repository root: python -m storyseq.pretrain_text [--epochs 8] [--extra groundcap]
"""
...
from storyseq.components import TextDecoder
from storyseq.data import load_split, tokenizer
...
def main() -> None:
    ...
    if args.extra == "groundcap":
        gc = torch.load(ROOT / "cache" / "groundcap.pt")["ids"].to(DEVICE)

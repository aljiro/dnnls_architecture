"""Data access for every level, built on the caches written by storyseq/precompute/*.

cache/{split}.pt holds, per story and frame (up to MAX_FRAMES frames):
...
for the text decoder's teacher forcing), cached as cache/tokens_{split}.pt.
"""
...
def model_kwargs(model, batch: dict) -> dict:
    """The optional forward() arguments a model's configuration needs, taken from a gathered batch."""
    cfg = model.cfg
    kw = {}
    if cfg.annotations:
        kw.update(set_emb=batch["set_emb"], ent_slot=batch["ent_slot"], chars_in=batch["chars_in"],
                  slot_name_ids=batch["slot_name_ids"])
